...
// tweet actions
$('.tweetwrapper').hover(
			 function () { $(this).addClass('activetweet') },
			 function () { $(this).removeClass('activetweet') }
)

//$('.container').prepend("<div id='scrollbar-track'><div id='scrollbar'></div></div>");

function scrollBar() {

	var viewportHeight = $(window).height();
	var docuHeight = $('.container').height();
...
$('#scrollbar').draggable({
	...
	drag: function () {
		...
		$('.container').scrollTop(scrollbarTop);
...
function setSlider($scrollpane) {//$scrollpane is the div to be scroll
	...
	if (difference > 0)//if the scrollbar is needed, set it up...
	{
		...
		$scrollpane.find('.slider-vertical').slider({
			...
			slide: function (event, ui) {
				...
				$scrollpane.find('.ui-slider-range').height(ui.value + '%');//set the height of the range element
			},
		...
		if (handleImage) {
			//$(".ui-slider-handle").append('<img class="scrollbar-top" src="includes/trans.png"/>');
			//$(".ui-slider-handle").append('<img class="scrollbar-bottom" src="includes/trans.png"/>');
			$scrollpane.find(".ui-slider-handle").append('<img class="scrollbar-grip" src="http://dev.diono.com/modules/mod_get_tweets/images/slide-grip.png"/>');
	...
	$scrollpane.find(".ui-slider").click(function (event) {//stop any clicks on the slider propagating through to the code below
		event.stopPropagation();
	});

	$scrollpane.find(".slider-wrap").click(function (event) {//clicks on the wrap outside the slider range
